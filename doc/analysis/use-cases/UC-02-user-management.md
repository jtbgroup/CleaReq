...
- Spring Security authorities are derived from all roles in `app_user_roles`; a user with `ADMIN`, `POPULATION_MANAGER`, and `SECURITY_OFFICER` receives the corresponding `ROLE_ADMIN`, `ROLE_POPULATION_MANAGER`, and `ROLE_SECURITY_OFFICER` authorities.